...
We note that the $\Gamma(1, 200)$ priors on $h_0$ and $h_1$ are conjugate to the Poisson process likelihood.
For fixed $s_1$ the likelihood factorises, so each height can be analytically marginalised:
$\int_0^\infty h_0^{n_0} e^{-h_0(s_1+\beta)}\,\beta^\alpha\,\mathrm{d}h_0 = \beta^\alpha\,\Gamma(\alpha+n_0)\,/\,(\beta+s_1)^{\alpha+n_0}$,
and similarly for $h_1$.
This yields a one-dimensional marginal posterior on $s_1$ alone, from which $h_0$ and $h_1$ samples can be drawn conditionally as $h_0 \mid s_1 \sim \Gamma(\alpha + n_0,\;\beta + s_1)$.
Such a collapsed (Rao--Blackwellised) approach would reduce the sampling problem from three dimensions to one, but we retain the full \texttt{emcee} sampler here for generality.
...
The Savage--Dickey density ratio provides the Bayes factor between nested models without computing evidences directly.
...
\]
i.e.\ the ratio of the marginal posterior to prior density of $\delta$ evaluated at $\delta = 0$, both computed under $M_1$.
This avoids any explicit evidence calculation.

The Savage--Dickey ratio requires evaluating the numerator $p(\delta = 0 \mid \{I_i\}, M_1)$.
In practice this density must be estimated from MCMC samples drawn from the $M_1$ posterior (e.g.\ via a histogram or kernel density estimate at $\delta = 0$).
However, the $M_1$ posterior strongly favours distinct rates before and after the change point: the marginal posterior of $\delta$ concentrates around $\delta \approx 0.006$, far from zero (Figure~\ref{fig:q4b}).
Virtually no MCMC samples fall near $\delta = 0$, so any density estimate there is dominated by noise.

If evaluated, the ratio would give $B_{01} \approx 0$ (i.e.\ $B_{10} \to \infty$), qualitatively confirming $M_1$ is favoured.
However, the KDE value at $\delta = 0$ depends entirely on bandwidth and random fluctuations and could vary by many orders of magnitude, making the Bayes factor estimate quantitatively useless.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{figs/q4b_savage_dickey.pdf}
  \caption{Marginal posterior of $\delta = h_0 - h_1$ under $M_1$. The nested point $\delta = 0$ lies deep in the tail where no MCMC samples reach, making the Savage--Dickey numerator impossible to estimate reliably.}
  \label{fig:q4b}
\end{figure}
